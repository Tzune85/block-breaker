...
function checkifPlayerWon() {
  ...
  if (player.inventory.diamond >= diamondToCollect || 
...
function paintIrons(node, index) {
  let { x, y , density } = node
  let noiseAmount = density * y
  let yLevel = HEIGHT - y
  if (noiseAmount > yLevel * 1.5  && diamondChance > random(100)){
...
function placeBlock(node) {
  ...
  if (playerHasEnoughRisources(blockType)) {
    player.inventory[blockType]--
    setAndDrawBlockType(node, blockType)
  }
}

function playerHasEnoughRisources(blockType) {
  switch (blockType) {
    case grass:
